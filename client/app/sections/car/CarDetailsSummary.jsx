...
import { addToCartRequest } from "../../actions/car";

// ----------------------------------------------------------------------

function CarDetailsSummary({ car, addToCart }) {
  const { id, title, description, available, color } = car;
  ...
  const { reset, watch, control, setValue, handleSubmit, getValues } = methods;
  ...
  const handleAddCart = handleSubmit(async (data) => {
    const cartData = {
      ...
      image_id: color[data.colorsPicker].tone[data.selectedImage].image_id,
    };
